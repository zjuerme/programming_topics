...
double nowangle=0;//当前角度

#define PI 3.1415926535 

void move(double distance){
	MovePen((double)GetCurrentX()+distance*sin(nowangle*PI/180),(double)GetCurrentY()+distance*cos(nowangle*PI/180));
}

void forward(double distance){
	DrawLine(distance*sin(nowangle*PI/180),distance*cos(nowangle*PI/180));
}

void turn(double angle){
	nowangle=nowangle+angle;
} 
