...
extern char * selectedLabel;
extern int labelstate;
...
void uiGetMouse(int x, int y, int button, int event)
{
	 ...
	 case BUTTON_DOWN:
		 ...
		 /*超链接的实现函数，但是现在存在不能包含文件的问题 
		 int i;
		 for(i=1;i<=hyperCnt;i++){
		 	if(x>hyperLink[i].x1&&x<hyperLink[i].x2&&y>hyperLink[i].y1&&y<hyperLink[i].y2){
		 		strcpy(lables[nowlable].lableurl,hyperLink[i].url);
		 		DoSearch();
			 }
		 }
		 */
...
int textbox(int id, double x, double y, double w, double h, char textbuf[], int buflen)
{
	...
	if ( gs_UIState.kbdItem == id )
	{
		...
		if (gs_UIState.charInput >= 32 && gs_UIState.charInput < 127 && len+1 < buflen ) {
		 ...
		 // 中文输入 
    	static int flag = 0;
    	if (gs_UIState.charInput < 0 && len+2 < buflen && flag==0 ) {
      		textbuf[len] = gs_UIState.charInput;
      		textbuf[++len] = 0;
      		textChanged = 1;
      		flag = 1;} 
    else if (gs_UIState.charInput < 0 && flag == 1 ){
      textbuf[len] = gs_UIState.charInput;
      textbuf[++len] = 0;
      textChanged = 1;
      flag = 0;
